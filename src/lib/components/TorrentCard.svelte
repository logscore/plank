...
>
    ...
    <div class="absolute inset-0 rounded-lg overflow-hidden">
        ...
        {#if item.voteAverage && item.voteAverage > 0}
            <div
                class="absolute top-2 right-2 z-10 bg-black/80 text-yellow-400 px-2 py-0.5 rounded text-xs font-medium flex items-center gap-1 group-hover:hidden"
            >
    ...
    <div
        class={cn(
			'absolute inset-0 p-3 rounded-lg flex flex-col justify-between transition-all duration-300 ease-out bg-black/70 backdrop-blur-sm',
			isMobileActive
				? 'opacity-100 pointer-events-auto'
				: 'opacity-0 pointer-events-none group-hover:opacity-100 group-hover:pointer-events-auto'
		)}
    >
        <div class="space-y-1 overflow-hidden flex-1 min-h-0 flex flex-col">
            <h4 class="font-bold text-sm leading-tight text-white line-clamp-2">{item.title}</h4>
            ...
            {#if item.genres.length > 0}
                <div class="flex flex-wrap gap-1 pt-1">
                    {#each item.genres.slice(0, 2) as genre}
                        <span
                            class="px-1.5 py-0.5 bg-zinc-700/80 text-zinc-300 text-[10px] rounded"
                        >
                            {genre}
                        </span>
                    {/each}
            ...
            {#if item.overview}
                <p class="text-[10px] text-zinc-400 line-clamp-3 pt-1">{item.overview}</p>
            {/if}
        </div>
