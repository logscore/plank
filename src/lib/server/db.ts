...
export const movies = {
	...
	getIncompleteDownloads(): Movie[] {
		return db
			.select()
			.from(moviesTable)
			.where(sql`${moviesTable.status} IN ('downloading', 'added')`)
			.all();
	},
	...
	getById(id: string): Movie | undefined {
		return db.select().from(moviesTable).where(eq(moviesTable.id, id)).get();
	},
	...
	updateProgress(
		...
		db.update(moviesTable).set({ progress, status }).where(eq(moviesTable.id, id)).run();
	},
	...
	updateLastPlayed(id: string) {
		db.update(moviesTable).set({ lastPlayedAt: new Date() }).where(eq(moviesTable.id, id)).run();
	},
