...
describe('ImageStorageService', () => {
    ...
    it('should save a file from buffer', async () => {
        ...
        expect(fs.mkdir).toHaveBeenCalledWith(
            path.normalize('/mock/data/library/123'), 
            { recursive: true }
        );
    ...
    it('should handle fetch errors in saveFromUrl', async () => {
        ...
        await expect(service.saveFromUrl('a', 'b', 'c', 'http://bad.url'))
            .rejects.toThrow('Failed to fetch image from http://bad.url: Not Found');
    });
