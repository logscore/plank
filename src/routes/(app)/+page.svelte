...
<div class="container mx-auto px-4 py-8">
  ...
  <div class="flex gap-2 mb-6 border-b border-border pb-4">
    ...
    >
      ...
      Movies
      {#if movies.length > 0}
        <span class="ml-2 text-xs bg-accent px-2 py-0.5 rounded-full"> {movies.length} </span>
      {/if}
    </Button>
    <Button variant={activeTab === 'tv' ? 'default' : 'ghost'} onclick={() => activeTab = 'tv'}>
      ...
      TV Shows
      {#if shows.length > 0}
        <span class="ml-2 text-xs bg-accent px-2 py-0.5 rounded-full"> {shows.length} </span>
      {/if}
    </Button>
  </div>
