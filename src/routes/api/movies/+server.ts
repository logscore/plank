...
export const POST: RequestHandler = async ({ request, locals }) => {
	...
	console.log(
		`[POST /api/movies] Parsed magnet - Title: "${title}", Year: ${year}, Infohash: ${infohash}`
	);
	...
	if (title && config.tmdb.apiKey) {
		try {
			...
			if (results.length > 0) {
				...
				console.log(
					`[POST /api/movies] Using TMDB result: "${basicResult.title}" (${basicResult.year}), fetching details...`
				);
		...
		console.log(
			`[POST /api/movies] Skipping TMDB search - title: "${title}", apiKey: ${!!config.tmdb.apiKey}`
		);
	}
